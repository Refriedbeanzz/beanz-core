...
public class InspectUniversePlayerRegistry {
  public static void main(String[] args) throws Exception {
    ...
    try (URLClassLoader cl = new URLClassLoader(new URL[]{jar.toURI().toURL()}); java.util.jar.JarFile jf = new java.util.jar.JarFile(jar)) {
      ...
      while (e.hasMoreElements()) {
        ...
        if (!name.toLowerCase().contains("player")) continue;
        try {
          Class<?> c = Class.forName(name, false, cl);
          for (Method m : c.getDeclaredMethods()) {
            String s = m.toString();
            if (s.contains("PlayerRef") && (s.contains("get") || s.contains("find") || s.contains("resolve"))) {
